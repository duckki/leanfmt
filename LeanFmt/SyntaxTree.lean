...
def nodeKindName : NodeKind → String
  ...
  | .tacticAttachedProof => "LeanFmt.SyntaxTree.NodeKind.tacticAttachedProof"
  ...
  | .modifiedDeclaration => "LeanFmt.SyntaxTree.NodeKind.modifiedDeclaration"
...
private def exposeAssignedProofBody? (split : OwnedProofBodySplit)
  ...
  let introducerStartsNewLine := !sharesSourceLineWith split.before split.body
  if proofBody.containsIntrinsicTacticLayoutOwner && !introducerStartsNewLine then
    none
  let bodySplit :=
...
private def splitAssignedProofBody? (kind : NodeKind) (children : Array Tree)
    : Option OwnedProofBodySplit := do
  let proofIndex ←
    children.findIdx?
      fun
      | .node (.raw proofKind) _ =>
          proofKind == `Lean.Parser.Term.byTactic
          || proofKind == `Lean.Parser.Term.byTactic'
      | _ => false
  let proof ← children[proofIndex]?
  let .node proofKind proofChildren := proof | none
  let bodyIndex ←
    proofChildren.findIdx?
      fun
      | .node (.proofBody _) _ => true
      | _ => false
  let proofBody ← proofChildren[bodyIndex]?
  let bodySplit :=
    splitNodeAroundChild proofKind proofChildren bodyIndex
      { before := .missing, body := proofBody, after := .missing }
  if bodySplit.after != .missing then
    none
  let declarationSplit :=
    splitNodeAroundChild kind children proofIndex
      { before := .missing, body := proof, after := .missing }
  if declarationSplit.before.tokens.back?.map (·.lexeme) != some ":=" then
    none
  some
    {
      before := .node .suffixGroup #[declarationSplit.before, bodySplit.before]
      body := proofBody
      after := declarationSplit.after
    }

private def regroupAssignedProofDeclaration? (kind : NodeKind) (children : Array Tree)
    : Option Tree := do
  let isLocalDeclaration :=
    match kind with
    | .raw rawKind | .tactic rawKind _ _ _ _ =>
        (nodeKindName (.raw rawKind)).endsWith "Decl"
    | _ => false
  ...
  let proofIndex ←
    children.findIdx?
      ...
      | _ => false
  let assignmentIndex ←
    (List.range proofIndex).reverse.find?
      fun index =>
        children[index]?.any
          fun
          | .leaf token => token.lexeme == ":="
          | _ => false
  let assignment ← children[assignmentIndex]?
  let proof ← children[proofIndex]?
  if Tree.sharesSourceLineWith assignment proof then
    none
  let split ← splitAssignedProofBody? kind children
  some <| .node .tacticAssignmentProof #[split.before, split.body, split.after]
...
private def annotateTacticNode
  ...
  if summary.isOwner then
    ...
    let assignedSplit? := splitAssignedProofBody? (.raw kind) children
    let split? :=
      assignedSplit?.orElse
        fun _ =>
          splitTrailingOwnedProofBody? (.node (.raw `null) children) isSpacedApplication
    match split? with
    | some split =>
        let (groupKind, split) :=
          match assignedSplit? with
          | some _ => (NodeKind.tacticAssignmentProof, split)
          | none =>
        ...
        | .node _ shellChildren =>
            let shell :=
              if groupKind == .tacticAssignmentProof then
                .node (.tactic kind summary.containsSequence false false false)
                  #[.node .suffixGroup shellChildren]
                ...
                .node (.tactic kind summary.containsSequence false false false)
                  shellChildren
            let groupKind :=
              match groupKind, shell with
              | .suffixGroup, .node .parserOwnedHeader _ =>
                  if split.body.proofBodyHasMultipleTactics then
                    .suffixGroup
                  else
                    .tacticAttachedProof
              | _, _ => groupKind
...
private partial def annotateTacticSequenceEntry (parserLayout : ParserLayout.Facts)
  ...
  | .node (.raw `null) children =>
      let children :=
        children.foldl
          (fun result child =>
            let child := annotateTacticSequenceEntry parserLayout child
...
def annotateTacticTree (tree : Tree) (parserLayout : ParserLayout.Facts := {}) : Tree :=
  match tree with
  | tree@(.node (.raw kind) children) =>
      let kindName := nodeKindName (.raw kind)
      if isTacticSequenceKind kind then
        annotateTacticNode kind (children.map (annotateTacticSequenceEntry parserLayout))
...
def normalizesStructuralBoundary (parent : Tree) (childIndex : Nat) (right : Token)
    : Bool :=
  match parent with
  | .node (.infixChain kind) children =>
      let startsAtRight :=
        (children[childIndex]?.bind Tree.firstToken?).any
          fun first => first.span.start == right.span.start
      kind != `Lean.Parser.Term.proj && 0 < childIndex && startsAtRight
  | .node (.raw `Lean.Parser.Term.typeAscription) children =>
      let startsAtRight :=
        (children[childIndex]?.bind Tree.firstToken?).any
          fun first => first.span.start == right.span.start
      0 < childIndex && startsAtRight
  | _ => false
...
private def regroupGeneratedPostfix?
    (parserLayout : ParserLayout.Facts)
    (kind : SyntaxNodeKind) (children : Array Tree)
    : Option Tree := do
  if !isGeneratedTermKind kind || (parserLayout.infixPrecedence? kind).isNone then
    none
  let content := children.filter fun child => child.firstToken?.isSome
  let #[operand, suffix] := content | none
  let suffixToken ← directLeafAtomToken? suffix
  let operandToken ← operand.lastToken?
  if !sourceTokensAreAdjacent operandToken suffixToken then
    none
  some <| .node .suffixGroup #[operand, suffix]
...
private def regroupSpacedApplication?
  ...
  if !generated && !parserLayout.isSpacedApplication kind then
    none
  let contentIndexes :=
    (List.range children.size).filter
...
def infixKindsSharePrecedence
    ...
    : Bool :=
  left == right
  || match parserLayout.infixPrecedence? left, parserLayout.infixPrecedence? right with
...
def appendInfixParts
  ...
  | .node (.infixChain childKind) children =>
      if infixKindsSharePrecedence parserLayout kind childKind then
        parts ++ children
      else
        parts.push tree
  | _ => parts.push tree
...
private def regroupPrefixedApplication? (children : Array Tree)
    : Option (Array Tree) := do
  let #[prefixTree, .node (.raw `null) operandChildren] := children | none
  let content := operandChildren.filter fun child => child.firstToken?.isSome
  let [operand] := content.toList | none
  some #[attachPrefixToOperandHead prefixTree operand]
...
private partial def unwrapAttachedDoRemainder : Tree → Tree
  | tree@(.node (.raw kind) children) =>
      if kind == `Lean.Parser.Term.doNested then
        match singleContentChild? children with
        | some body => body
        ...
        match singleContentChild? children with
        | some child =>
            if (attachedDoTree? child).isSome then
              unwrapAttachedDoRemainder child
            else
              tree
        | none => tree
      else
        ...
        | some left, some right =>
            if right.firstToken?.any accepts then
              ...
              | some (introducer, body) =>
                  let body :=
                    if unwrapNestedDo
                        && introducer.firstToken?.any (·.lexeme == "do") then
                      unwrapAttachedDoRemainder body
                    else
                      unwrapAttachedBodyRemainder body
...
private def regroupParserOwnedBody?
    (kind : SyntaxNodeKind) (policy : ParserLayout.OwnedBodyPolicy)
    (children : Array Tree)
    : Option Tree := do
...
private def tacticEndsWithDetachedBodySuffix (parserLayout : ParserLayout.Facts)
    : Tree → Bool
  | tree@(.node (.raw kind) _)
  | tree@(.node (.tactic kind _ _ _ _) _) =>
      tree.lastToken?.any
        fun token =>
          ((parserLayout.ownedBody? kind).any
...
private def splitDetachedParserOwnedSuffix?
    (parserLayout : ParserLayout.Facts) (tree : Tree)
    : Option (Tree × Tree) := do
  let suffixes ←
    match tree with
    | .node (.raw kind) _
    | .node (.tactic kind _ _ _ _) _ =>
        (parserLayout.ownedBody? kind).map (·.suffixes)
...
private partial def regroupDetachedParserOwnedTacticBodiesCore
  ...
  | .node kind children =>
      let regroupedChildren :=
        children.map (regroupDetachedParserOwnedTacticBodiesCore parserLayout)
      ...
      let (children, containsDetached) :=
        if kind == .parserOwnedBody || kind == .tacticAttachedProof then
          (children, containsDetached)
        else
          loop #[] containsDetached children.toList
      let (children, containsDetached) :=
      ...
      | _, _ =>
          match regroupDirectParserOwnedTacticBody? parserLayout kind children with
          | some tree => (tree, true)
...
private def regroupDetachedParserOwnedTacticBodies
    (parserLayout : ParserLayout.Facts) (tree : Tree)
    : Tree :=
  (regroupDetachedParserOwnedTacticBodiesCore parserLayout tree).1
...
private def regroupApplicationAttachedBody? (children : Array Tree) : Option Tree := do
  let contentIndexes :=
    (List.range children.size).filter
      fun index => children[index]?.bind Tree.firstToken? |>.isSome
  if contentIndexes.length < 2 then
    none
  let bodyIndex ← contentIndexes.getLast?
  let wrappedBody ← children[bodyIndex]?
  let (introducer, body) ←
    splitLeadingToken?
      (fun token => token.lexeme == "do" || token.lexeme == "by")
      wrappedBody
  let body := unwrapAttachedBodyRemainder body
  if body.firstToken?.isNone then
    none
  let header := .node .application (children.set! bodyIndex introducer)
  some <| .node .parserOwnedBody #[header, body]

private partial def singleInitializeCommandOperand? : Tree → Option Tree
  | .node (.raw kind) children =>
      if kind == `null
          || kind == `Lean.Parser.Term.doSeqIndent
          || kind == `Lean.Parser.Term.doSeqItem
          || kind == `Lean.Parser.Term.doExpr then
        singleContentChild? children >>= singleInitializeCommandOperand?
      else
        some <| .node (.raw kind) children
  | tree => some tree

def regroupInitialize? (children : Array Tree) : Option Tree :=
  let assigned? : Option Tree := do
    let modifiers ← children[0]?
    let keyword ← children[1]?
    let .node (.raw `null) headerAndAssignment ← children[2]? | none
    let body ← children[3]?
    let body :=
      (attachedDoTree? body).orElse (fun _ => singleInitializeCommandOperand? body)
      |>.getD body
    let assignmentIndex ←
  ...
  assigned?.orElse
    fun _ => do
      ...
      let keyword ← children[1]?
      let body ← children[3]?
      let operand ← singleInitializeCommandOperand? body
      if Tree.sharesSourceLineWith keyword operand then
        some
        <| .node (.raw `Lean.Parser.Command.initialize)
            (#[modifiers, attachPrefixToOperandHead keyword operand]
              ++ childrenRange children 4 children.size)
      else
        some <| .node (.raw `Lean.Parser.Command.initialize) children
...
def modifiedDeclarationTree (modifiers declaration : Tree) : Tree :=
  .node .modifiedDeclaration #[modifiers, declaration]
...
def regroupStructCtor (children : Array Tree) : Tree :=
  ...
  | some modifiers =>
      ...
      | none =>
          if modifiers.firstToken?.isSome then
            modifiedDeclarationTree modifiers command
          else
            command
  | none => command
...
def regroupDeclarationValueCommand (kind : SyntaxNodeKind) (children : Array Tree)
  ...
  | none =>
      match splitLeadingDeclarationModifiers? command with
      | some (modifiers, command) => modifiedDeclarationTree modifiers command
      | none => command
    ...
    | none =>
        ...
        | none =>
            if (Tree.firstToken? modifiers).isSome then
              some <| modifiedDeclarationTree modifiers declaration
            else
              none
...
def regroupDoFallbackChildren? (children : Array Tree) : Option (Array Tree) := do
  ...
  let clause :=
    .node .doFallbackClause
      (regroupAttachedBodyIntroducerChildren #[pipe, fallback] (unwrapNestedDo := true))
...
partial def singleAnnotatedParserOwnedBody? : Tree → Option Tree
  | tree@(.node .parserOwnedBody children) =>
      if (children[0]?).any (Tree.containsNodeKind .parserOwnedHeader)
          && (children[1]?).any
              fun
              | .node .parserOwnedTacticBody _ => true
              | _ => false then
        ...
        none
  | .node kind children =>
      if kind == .proofBody false
          || kind == .proofBody true
          || kind == .raw `null
          || kind == .tactic `Lean.Parser.Tactic.tacticSeq true false true false
          || kind
              == .tactic `Lean.Parser.Tactic.tacticSeq1Indented true false true false then
        singleContentChild? children >>= singleAnnotatedParserOwnedBody?
      else
        none
  | _ => none
...
def regroupByTacticChildren (children : Array Tree) : Array Tree :=
  ...
  | some byKeyword =>
      ...
      match singleAnnotatedParserOwnedBody? body with
      | some (.node .parserOwnedBody #[header, ownedBody]) =>
          #[byKeyword, .node .tacticAttachedProof #[header, ownedBody]]
      | none =>
...
def regroupOtherRawNode (kind : SyntaxNodeKind) (children : Array Tree) : Tree :=
  ...
  else if kind == `Lean.Parser.Term.doFor || kind == `Lean.Parser.Term.termFor then
...
private partial def appendToTrailingSuffixGroup? (suffix : Tree) : Tree → Option Tree
  | .node .suffixGroup children =>
      some <| .node .suffixGroup (children.push suffix)
  | .node kind children => do
      let index ← previousContentIndex? children children.size
      let child ← children[index]?
      let child ← appendToTrailingSuffixGroup? suffix child
      some <| .node kind (children.set! index child)
  | _ => none

private def attachEliminationHeaderSuffix (children : Array Tree) (suffix : Tree)
    : Array Tree :=
  match previousContentIndex? children children.size with
  | some index =>
      match children[index]? with
      | some child =>
          let child :=
            (appendToTrailingSuffixGroup? suffix child).getD
              (.node .suffixGroup #[child, suffix])
          children.set! index child
      | none => children
  | none => children

private def unwrapProofBodyEnvelope : Tree → Tree
  | .node (.proofBody _) #[body] => body
  | tree => tree
...
def regroupCasesChildren (children : Array Tree) : Option (Array Tree) := do
  ...
  | some (defaultAlternative, explicitAlternatives) =>
      let (headerChildren, defaultAlternative) :=
        match splitLeadingDelimitedSequenceOpener? defaultAlternative with
        | some (opener, defaultAlternative) =>
            (
              attachEliminationHeaderSuffix headerChildren opener,
              unwrapProofBodyEnvelope defaultAlternative
            )
        | none => (headerChildren, defaultAlternative)
      let header :=
...
def regroupInductionChildren (children : Array Tree) : Option (Array Tree) := do
  ...
  let trailingChildren := childrenRange children (alternativesIndex + 1) children.size
  let (headerChildren, bodyChildren) :=
    match splitDefaultTacticAlternative? alternatives with
    | some (defaultAlternative, explicitAlternatives) =>
        match splitLeadingDelimitedSequenceOpener? defaultAlternative with
        | some (opener, defaultAlternative) =>
            (
              attachEliminationHeaderSuffix headerChildren opener,
              #[unwrapProofBodyEnvelope defaultAlternative, explicitAlternatives]
            )
        | none => (headerChildren, #[defaultAlternative, explicitAlternatives])
    | none => (headerChildren, #[alternatives])
...
def regroupRawNode
    ...
    : Tree :=
  if kind == `choice then
    match children.filter fun child => child.firstToken?.isSome with
    | #[child@(.node (.delimitedCollection _) _)] => child
    | _ => .node (.raw kind) children
  else if kind == `null then
    ...
    regroupIfThenElseChain kind children
          ...
          || kind == `Lean.Parser.Term.fromTerm
          || kind == `Lean.Parser.Term.doRepeat then
    let children :=
      ...
      else if kind == `Lean.Parser.Term.doRepeat then
        regroupAttachedBodyIntroducerChildren children (unwrapNestedDo := true)
      else
        regroupAttachedBodyIntroducerChildren children
    ...
    | some receiver, some operator, some head =>
        ...
        let parts := appendInfixParts parserLayout kind #[] receiver
  ...
  else if kind == `Lean.Parser.Command.printAxioms then
    match children[0]?, children[1]?, children[2]? with
    | some command, some keyword, some name =>
        .node .application #[.node .suffixGroup #[command, keyword], name]
    | _, _, _ => .node (.raw kind) children
...
private partial def regroupTreeWithPrecedencesInContext
  ...
  | .node (.raw kind) children =>
      ...
      let generatedDelimiter? :=
        let hasCollectionSeparator :=
          children.any
            fun child =>
              child.singleToken?.any
                fun token => token.lexeme == "," || token.lexeme == ";"
        if isGeneratedTermKind kind
            && parserLayout.hasPrintingBreak kind
            && hasCollectionSeparator then
      ...
      let outerDelimiter? :=
        if isDelimitedCollection || isGeneratedTermKind kind then
          ...
          outerDelimiterKind? children
      let tree :=
        if isDelimitedCollection
            || outerDelimiter? == some .bracket
            || generatedDelimiter?.isSome then
          ...
          match generatedDelimiter? with
          | some delimiter => .node (.delimitedCollection delimiter) children
          | none =>
          ...
          regroupRawNode parserLayout (!isTacticSequenceEntry) kind
...
def regroupTreeWithPrecedences (parserLayout : ParserLayout.Facts := {}) : Tree → Tree :=
  fun tree =>
    regroupSpacedTacticFirstOperands
    <| regroupDetachedParserOwnedTacticBodies parserLayout
    <| regroupTreeWithPrecedencesInContext parserLayout false tree
...
def regroupTopLevelCommandAnnotations (tree : Tree) : Tree :=
  match tree with
  | .node .annotatedDeclaration _ | .node .modifiedDeclaration _ => tree
  | _ =>
      match splitDeclarationAnnotations? tree with
      | some (annotations, command) =>
          annotatedDeclarationTreeForCommand annotations command
...
private def annotateTopLevelCommand : Tree → Tree
  ...
  | .node .modifiedDeclaration children =>
      match children.back? with
      | some command@(.node (.raw _) _) =>
          .node .modifiedDeclaration
            (children.set! (children.size - 1) (annotateCommandContext command))
      | _ => .node .modifiedDeclaration children
...
private partial def regroupAssignedProofDeclarations : Tree → Tree
  | .missing => .missing
  | .leaf token => .leaf token
  | .node kind children =>
      let children := children.map regroupAssignedProofDeclarations
      let tree := .node kind children
      match kind with
      | .raw _ | .tactic _ _ _ _ _ =>
...
def checkParserMessages (messages : MessageLog) : IO Unit := do
...
def parseModuleStringWithEnv (env : Environment) (source fileName : String := "<input>")
  ...
  let tree :=
    extractTree source parsed.rawSyntax parsed.letBodyParserFacts parsed.parserLayout
  pure
    {
      source
