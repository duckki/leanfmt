...
def assertNonSourceLexemesPreserveModule (env : Lean.Environment) : IO Unit := do
  let source := "def generatedParserOutput := 0\n"
  let parsed ←
    SyntaxTree.parseModuleStringWithEnv env source "non-source-parser-lexeme.lean"
  let synthetic :=
    SyntaxTree.Tree.leaf
    <| SyntaxTree.tokenOfNone .atom .anonymous "generated-non-source-token"
  let tree := SyntaxTree.Tree.node (.raw `null) #[synthetic, parsed.tree]
  let moduleTree : SyntaxTree.Module := { parsed with tree, tokens := tree.tokens }
  assertTrue "a non-source parser lexeme is detected" moduleTree.containsNonSourceLexemes
  assertEq "a module with non-source parser lexemes is preserved"
    source (Formatter.formatModule moduleTree)
  assertTrue "a non-source parser tree does not report unreachable rules"
...
def assertSetOptionInBreaksAfterIn (env : Lean.Environment) : IO Unit := do
  ...
  let termSource :=
    "def scopedOptionTerm : Nat × Nat :=\n"
    ++ "  set_option backward.isDefEq.respectTransparency false in\n"
    ++ "    { fst := 0, snd := 1 }\n"
  let termModule ←
    SyntaxTree.parseModuleStringWithEnv env termSource "set-option-term.lean"
  assertTrue "term set_option has complete rule coverage"
    (Formatter.Diagnostics.missingRuleOccurrencesForModule termModule).isEmpty
...
def assertOpenCommandListBreaksFromCommandBase (env : Lean.Environment) : IO Unit := do
  let source :=
  ...
  assertEq "open command list breaks from the command base" expected formatted

  let openOnlySource :=
    "open HexPolyZMathlib (toPolynomial coeff_toPolynomial toPolynomial_add "
    ++ "toPolynomial_sub toPolynomial_mul toPolynomial_C equiv)\n"
  let openOnlyExpected :=
    "open HexPolyZMathlib (\n"
    ++ "  toPolynomial coeff_toPolynomial toPolynomial_add toPolynomial_sub "
    ++ "toPolynomial_mul toPolynomial_C\n"
    ++ "    equiv\n"
    ++ "  )\n"
  let openOnlyResult ←
    Formatter.formatSourceWithEnvDetailed env openOnlySource
      "open-only-command-list.lean" { lineWidth := 100 }
  assertTrue "parenthesized open list does not fall back" (!openOnlyResult.fellBack)
  assertEq "parenthesized open list exposes identifier flow"
    openOnlyExpected openOnlyResult.formatted
  assertTrue "parenthesized open list fits its width"
    (Formatter.linesFit openOnlyResult.formatted 100)
  assertTrue "parenthesized open list preserves code"
    (← codePreservedIgnoringWhitespace env openOnlySource openOnlyResult.formatted)
  let openOnlyAgain ←
    Formatter.formatSourceWithEnv env openOnlyResult.formatted
      "open-only-command-list-formatted.lean" { lineWidth := 100 }
  assertEq "parenthesized open list is idempotent" openOnlyResult.formatted openOnlyAgain

def assertPrintAxiomsUsesCommandHead (env : Lean.Environment) : IO Unit := do
  let source :=
    "#print axioms AutomorphicForm.IsArithBoundedGenuineCuspRealizableVia.isArithGenuineCuspRealizableVia\n"
  let expected :=
    "#print axioms\n"
    ++ "  AutomorphicForm.IsArithBoundedGenuineCuspRealizableVia.isArithGenuineCuspRealizableVia\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source "print-axioms-command-head.lean"
      { lineWidth := 90 }
  assertEq "print axioms keeps its command head and breaks before a long name"
    expected formatted
...
def assertDoLetElseBreaks (env : Lean.Environment) : IO Unit := do
  ...
  assertEq "multiline do-let fallback keeps continuation at body base"
    multilineFallbackExpected multilineFallbackFormatted
...
def assertAssignedProofKeepsByWithAssignment (env : Lean.Environment) : IO Unit := do
  let source :=
    "theorem assignedProofKeepsByWithAssignment (n : Nat) : True := by\n"
    ++ "  have veryLongLocalProofNameForAssignmentOwnership : True :=\n"
    ++ "    by\n"
    ++ "      induction n with\n"
    ++ "      | zero => exact True.intro\n"
    ++ "      | succ n ih => exact ih\n"
    ++ "  exact veryLongLocalProofNameForAssignmentOwnership\n"
  let expected :=
    "theorem assignedProofKeepsByWithAssignment (n : Nat) : True := by\n"
    ++ "  have veryLongLocalProofNameForAssignmentOwnership : True := by\n"
    ++ "    induction n with\n"
    ++ "    | zero => exact True.intro\n"
    ++ "    | succ n ih => exact ih\n"
    ++ "  exact veryLongLocalProofNameForAssignmentOwnership\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source
      "assigned-proof-keeps-by-with-assignment.lean" { lineWidth := 80 }
  assertEq "an assigned proof keeps by with the assignment token" expected formatted
  let quantifiedSource :=
    "theorem detachedLongAssignedProof : True := by\n"
    ++ "  have key : ∀ (value : VeryLongTypeName) {first second : Nat}, first ≤ second →\n"
    ++ "      (∀ x, veryLongPredicateName value x first) →\n"
    ++ "      (∀ x y, anotherVeryLongPredicateName value x y second) →\n"
    ++ "      finalVeryLongPredicateName value first second :=\n"
    ++ "    by\n"
    ++ "      intro value first second hfirst hpredicate hanother\n"
    ++ "      exact finalProof\n"
    ++ "  exact key\n"
  let quantifiedExpected :=
    "theorem detachedLongAssignedProof : True := by\n"
    ++ "  have key\n"
    ++ "      : ∀ (value : VeryLongTypeName) {first second : Nat},\n"
    ++ "          first ≤ second\n"
    ++ "          → (∀ x, veryLongPredicateName value x first)\n"
    ++ "          → (∀ x y, anotherVeryLongPredicateName value x y second)\n"
    ++ "          → finalVeryLongPredicateName value first second := by\n"
    ++ "    intro value first second hfirst hpredicate hanother\n"
    ++ "    exact finalProof\n"
    ++ "  exact key\n"
  let quantifiedResult ←
    Formatter.formatSourceWithEnvDetailed env quantifiedSource
      "quantified-assigned-proof-keeps-by.lean" { lineWidth := 80 }
  assertTrue "a quantified assigned proof does not fall back" (!quantifiedResult.fellBack)
  assertEq "a quantified assigned proof keeps by with the assignment token"
    quantifiedExpected quantifiedResult.formatted
  assertTrue "a quantified assigned proof preserves code"
    (← codePreservedIgnoringWhitespace env quantifiedSource quantifiedResult.formatted)
  let quantifiedAgain ←
    Formatter.formatSourceWithEnv env quantifiedResult.formatted
      "quantified-assigned-proof-keeps-by-formatted.lean" { lineWidth := 80 }
  assertEq "a quantified assigned proof is idempotent"
...
def assertTermTakingTacticsAttachOperandHead (baseEnv : Lean.Environment) : IO Unit := do
  ...
  let source :=
    ...
    ++ "  cases value with\n"
    ++ "  | zero =>\n"
    ++ "      exact veryLongExactProofFunctionForTacticLayout\n"
    ++ "        firstArgument secondArgument\n"
    ++ "      apply veryLongApplyProofFunctionForTacticLayout\n"
    ++ "        firstArgument secondArgument\n"
    ++ "      refine veryLongRefineProofFunctionForTacticLayout\n"
    ++ "        firstArgument secondArgument\n"
    ++ "      context_term_tactic veryLongExtensionFunctionForTacticLayout\n"
    ++ "        firstArgument secondArgument\n"
    ++ "  | succ value => exact True.intro\n"
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env source "term-taking-tactic-application.lean"
  for (tactic, kind)
      in [
  ...
  let combinatorSource :=
    ...
    ++ "  apply veryLongApplyProofFunction secondArgument\n"
  let combinatorTree ←
    SyntaxTree.parseModuleStringWithEnv env combinatorSource
      "combined-term-taking-tactic-application.lean"
  for (tactic, kind)
  ...
  assertTrue "term-taking tactic applications do not fall back" (!result.fellBack)
  assertTextContains "exact stays with its operand head" result.formatted
    "      exact veryLongExactProofFunctionForTacticLayout\n        firstArgument secondArgument\n"
  assertTextContains "apply stays with its operand head" result.formatted
    "      apply veryLongApplyProofFunctionForTacticLayout\n        firstArgument secondArgument\n"
  assertTextContains "refine stays with its operand head" result.formatted
    "      refine veryLongRefineProofFunctionForTacticLayout\n        firstArgument secondArgument\n"
  assertTextContains "extension tactic stays with its operand head" result.formatted
    "      context_term_tactic veryLongExtensionFunctionForTacticLayout\n        firstArgument secondArgument\n"
  assertTrue "term-taking tactic application formatting preserves code"
    (← codePreservedIgnoringWhitespace env source result.formatted)
  let formattedAgain ←
    Formatter.formatSourceWithEnv env result.formatted
...
def assertMovedInlineProofBodiesRemainParseable (env : Lean.Environment) : IO Unit := do
  ...
  let deepSource :=
    "structure DeepInlineProof where\n"
    ++ "  proofField : VeryLongOuterType\n"
    ++ "    (veryLongFirstFunction (veryLongFirstArgument first second)\n"
    ++ "      (veryLongSecondFunction\n"
    ++ "        (veryLongThirdFunction\n"
    ++ "          (by rw [show veryLongLeftHandSide first second = veryLongRightHandSide third fourth from equalityProof]\n"
    ++ "              exact finalProof))))\n"
    ++ "    remainingArgument\n"
  let deepExpected :=
    "structure DeepInlineProof where\n"
    ...
    ++ "            (veryLongThirdFunction\n"
    ++ "              (by\n"
    ++ "                rw [show veryLongLeftHandSide first second = veryLongRightHandSide third fourth from equalityProof]\n"
    ++ "                exact finalProof))))\n"
    ++ "        remainingArgument\n"
  let deepResult ←
    Formatter.formatSourceWithEnvDetailed env deepSource
      "deep-moved-inline-proof-body.lean" { lineWidth := 90 }
  assertTrue "a deeply moved inline proof does not fall back" (!deepResult.fellBack)
  assertEq "a deeply moved inline proof keeps tactic peers aligned"
    deepExpected deepResult.formatted
  assertTrue "a deeply moved inline proof preserves code"
    (← codePreservedIgnoringWhitespace env deepSource deepResult.formatted)
  let deepAgain ←
    Formatter.formatSourceWithEnv env deepResult.formatted
      "deep-moved-inline-proof-body-formatted.lean" { lineWidth := 90 }
  assertEq "a deeply moved inline proof is idempotent" deepResult.formatted deepAgain
...
def assertInitializeUsesStructuralDeclarationHeader (env : Lean.Environment)
  ...
  let suffixWidthSource :=
    "initialize sameRootRef : IO.Ref (Option (RefinedIsolation refinePoly × RefinedIsolation refinePoly)) ←\n"
    ++ "  IO.mkRef none\n"
  ...
  let suffixWidthFormatted ←
    Formatter.formatSourceWithEnv env suffixWidthSource
      "initialize-assignment-suffix-width.lean" { lineWidth := 100 }
  assertEq "initialize assignment suffix counts for signature fit"
...
def assertGeneratedPostfixMarkersStayAttached (env : Lean.Environment) : IO Unit := do
  let source :=
    ...
    ++ "      expected := by\n"
    ++ "  exact proof\n\n"
    ++ "abbrev generatedUnits :=\n"
    ++ "  ((veryLongNumeratorName firstArgument secondArgument) /\n"
    ++ "      (veryLongDenominatorName thirdArgument fourthArgument))ˣ\n\n"
    ++ "end Matrix\n"
  let result ←
  ...
  assertTextContains "generated postfix operator stays source-tight" result.formatted ")ˣ"
  assertTextLacks "generated postfix operator does not become spaced syntax"
    result.formatted ") ˣ"
  let formattedAgain ←
    Formatter.formatSourceWithEnv env result.formatted
...
def assertLetIExpressionKeepsBodyBreak (env : Lean.Environment) : IO Unit := do
  ...
  let parenthesizedSource :=
    "def parenthesizedLetISequence : Result :=\n"
    ++ "  (letI := veryLongFirstInstanceProvider firstArgument secondArgument\n"
    ++ "   letI := veryLongSecondInstanceProvider thirdArgument fourthArgument\n"
    ++ "   veryLongBodyExpression fifthArgument sixthArgument)\n"
  ...
  let parenthesizedResult ←
    Formatter.formatSourceWithEnvDetailed env parenthesizedSource
      "parenthesized-letI-sequence.lean" { lineWidth := 70 }
  assertTrue "a parenthesized letI sequence does not fall back"
    (!parenthesizedResult.fellBack)
  assertEq "a parenthesized letI sequence uses its containing base"
    parenthesizedExpected parenthesizedResult.formatted
  assertTrue "a parenthesized letI sequence preserves code"
    (← codePreservedIgnoringWhitespace env parenthesizedSource
        parenthesizedResult.formatted)
  let parenthesizedAgain ←
    Formatter.formatSourceWithEnv env parenthesizedResult.formatted
      "parenthesized-letI-sequence-formatted.lean" { lineWidth := 70 }
  assertEq "a parenthesized letI sequence is idempotent"
    parenthesizedResult.formatted parenthesizedAgain
...
def assertVariableBinderMovesAsPeerBeforeInternalBreak (env : Lean.Environment)
    : IO Unit := do
  let source :=
    "variable {q : Nat} [Fact q.Prime] {A : ExtremelyLongTypeConstructorNameForVariableLayout q} {N : Nat} [NeZero N] {k : Type} [Field k]\n"
  let expected :=
    "variable {q : Nat} [Fact q.Prime]\n"
    ++ "  {A : ExtremelyLongTypeConstructorNameForVariableLayout q} {N : Nat} [NeZero N]\n"
    ++ "  {k : Type} [Field k]\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source
      "variable-binder-peer-before-internal-break.lean" { lineWidth := 80 }
  assertEq "variable binder moves as a peer before using an internal break"
    expected formatted
  let narrowSource :=
    "variable {q : ℕ} [Fact q.Prime] {A : ValuationSubring (AlgebraicClosure ℚ)} {N : ℕ} [NeZero N]\n"
    ++ "  {k : Type*} [Field k]\n"
  let narrowExpected :=
    "variable {q : ℕ} [Fact q.Prime] {A : ValuationSubring (AlgebraicClosure ℚ)} {N : ℕ}\n"
    ++ "  [NeZero N]\n"
    ++ "  {k : Type*} [Field k]\n"
  let narrowFormatted ←
    Formatter.formatSourceWithEnv env narrowSource
      "variable-binder-narrow-peer-before-internal-break.lean" { lineWidth := 90 }
  assertEq "a narrow variable binder moves as a peer before using an internal break"
    narrowExpected narrowFormatted
...
def assertCommentedStructureItemKeepsFieldBase (env : Lean.Environment) : IO Unit := do
  let source :=
    "structure CommentedRecord where\n"
    ++ "  id : String\n"
    ++ "  first : Nat\n"
    ++ "  second : Nat\n"
    ++ "\n"
    ++ "def commentedRecords : List CommentedRecord := [\n"
    ++ "  { id := \"example\",\n"
    ++ "    first := 0,\n"
    ++ "    -- Keep this comment with the following field.\n"
    ++ "    second := 1 }\n"
    ++ "]\n"
  let expected :=
    "structure CommentedRecord where\n"
  ...
  let result ←
    Formatter.formatSourceWithEnvDetailed env source
      "commented-structure-item-field-base.lean"
  assertTrue "a commented structure item does not fall back" (!result.fellBack)
  assertEq "a commented structure item keeps the field base" expected result.formatted
  assertTrue "a commented structure item preserves code"
    (← codePreservedIgnoringWhitespace env source result.formatted)
  let formattedAgain ←
    Formatter.formatSourceWithEnv env result.formatted
      "commented-structure-item-field-base-formatted.lean"
  assertEq "commented structure item formatting is idempotent"
    result.formatted formattedAgain
...
def assertIntervalBinderBreaksBetweenRangeEndpoints (env : Lean.Environment)
    : IO Unit := do
  let source :=
    "def longIntervalIntegral :=\n"
    ++ "  ∫ s in lowerEndpointWithEnoughCharacters firstArgument.. upperEndpointWithEnoughCharacters secondArgument, integrandWithEnoughCharacters s\n"
  let expected :=
    "def longIntervalIntegral :=\n"
    ++ "  ∫ s in lowerEndpointWithEnoughCharacters firstArgument..\n"
    ++ "    upperEndpointWithEnoughCharacters secondArgument,\n"
    ++ "    integrandWithEnoughCharacters s\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source "interval-binder-range-break.lean"
      { lineWidth := 70 }
  assertEq "an interval binder breaks between its range endpoints" expected formatted
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env formatted
      "interval-binder-range-break-formatted.lean"
  assertTrue "interval binder formatting avoids overflow"
    (Formatter.Diagnostics.overflowOccurrences moduleTree { lineWidth := 70 }).isEmpty
...
def assertOrdinarySyntaxNormalizesSourceTightSpacing (env : Lean.Environment)
    : IO Unit := do
  let source := "def ordinaryTightSpacing := (3:Nat)*(x+y)\n"
  let expected := "def ordinaryTightSpacing := (3 : Nat) * (x + y)\n"
  let formatted ← Formatter.formatSourceWithEnv env source "ordinary-tight-spacing.lean"
  assertEq "ordinary syntax normalizes source-tight spacing" expected formatted

def assertMultiplicationChainsFlow (env : Lean.Environment) : IO Unit := do
  let source :=
    "def multiplicationChain := veryLongCoefficientWithEnoughCharacters firstArgument * firstShortFactor * secondShortFactor * thirdShortFactor * fourthShortFactor\n"
  let expected :=
    "def multiplicationChain :=\n"
    ++ "  veryLongCoefficientWithEnoughCharacters firstArgument * firstShortFactor\n"
    ++ "  * secondShortFactor * thirdShortFactor * fourthShortFactor\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source "multiplication-chain-flow.lean"
      { lineWidth := 78 }
  assertEq "multiplication chains pack fitting factors on continuation lines"
    expected formatted
...
def assertBlankSourceTriviaDoesNotDetachSyntax (env : Lean.Environment) : IO Unit := do
  let source :=
    "def blankBeforeInfix : Prop :=\n"
    ++ "  firstConditionWithEnoughCharactersForLayout ∧\n"
    ++ "\n"
    ++ "  secondConditionWithEnoughCharactersForLayout\n"
    ++ "\n"
    ++ "def blankAfterAscription :=\n"
    ++ "  (veryLongValueWithEnoughCharactersForLayout :\n"
    ++ "\n"
    ++ "    VeryLongTypeWithEnoughCharactersForLayout)\n"
    ++ "\n"
    ++ "theorem blankAfterResultColon\n"
    ++ "    (argument : VeryLongArgumentTypeWithEnoughCharactersForLayoutTesting) :\n"
    ++ "\n"
    ++ "    VeryLongReturnTypeWithEnoughCharactersForLayoutTesting := by\n"
    ++ "  exact proof\n"
  let expected :=
    "def blankBeforeInfix : Prop :=\n"
    ++ "  firstConditionWithEnoughCharactersForLayout\n"
    ...
    ++ "theorem blankAfterResultColon\n"
    ++ "    (argument\n"
    ++ "      : VeryLongArgumentTypeWithEnoughCharactersForLayoutTesting)\n"
    ++ "    : VeryLongReturnTypeWithEnoughCharactersForLayoutTesting := by\n"
    ++ "  exact proof\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source
      "blank-source-trivia-does-not-detach-syntax.lean" { lineWidth := 70 }
  assertEq "blank source trivia does not detach syntax" expected formatted
  assertEq "formatted output has no trailing horizontal whitespace"
    formatted (Formatter.SpaceRules.stripTrailingWhitespace formatted)
...
def assertMatchDiscriminantsFlowAfterCommas (env : Lean.Environment) : IO Unit := do
  ...
  let doMatchSource :=
    "def collectOffers state index := do\n"
    ++ "  match state.engine.applications[index]?, state.applications[index]?, state.engine.queued[index]? with\n"
    ++ "  | some application, some clock, some true => pure (application, clock)\n"
    ++ "  | _, _, _ => pure default\n"
  let doMatchExpected :=
    "def collectOffers state index := do\n"
    ...
    ++ "  | some application, some clock, some true => pure (application, clock)\n"
    ++ "  | _, _, _ => pure default\n"
  let doMatchFormatted ←
    Formatter.formatSourceWithEnv env doMatchSource "do-match-discriminants-flow.lean"
      { lineWidth := 100 }
  assertEq "do match discriminants use ordinary match flow"
    doMatchExpected doMatchFormatted
  assertTrue "do match discriminant formatting preserves code"
    (← codePreservedIgnoringWhitespace env doMatchSource doMatchFormatted)
  let doMatchModule ←
    SyntaxTree.parseModuleStringWithEnv env doMatchFormatted
      "do-match-discriminants-flow-formatted.lean"
  let discriminants ←
    match findTreeNode? .matchDiscriminants doMatchModule.tree with
    | some tree => pure tree
    | none => throw <| IO.userError "do match discriminants were not regrouped"
  let discriminantPlan :=
    Formatter.LayoutPlan.resolve {}
      (Formatter.LineBreakRules.Segment.ofTree discriminants)
  assertTrue "do match exposes every comma boundary"
    (discriminantPlan.breakPoints.length == 2)
  assertTrue "do match discriminant formatting avoids overflow"
    (Formatter.Diagnostics.overflowOccurrences doMatchModule { lineWidth := 100 }).isEmpty
  let doMatchAgain ←
    Formatter.formatSourceWithEnv env doMatchFormatted
      "do-match-discriminants-flow-formatted.lean" { lineWidth := 100 }
  assertEq "do match discriminant formatting is idempotent" doMatchFormatted doMatchAgain
  let parenthesizedSource :=
    "theorem resolveBoth\n"
    ++ "    : Result\n"
...
def assertStructureFieldBoundaryPreservesOneCleanBlankLine (env : Lean.Environment)
    : IO Unit := do
  let source := "structure PatchingDatum where\n\n  N : Type\n"
  let expected := "structure PatchingDatum where\n\n  N : Type\n"
  let formatted ←
    Formatter.formatSourceWithEnv env source "structure-field-clean-blank-boundary.lean"
  assertEq "structure field boundary preserves one clean blank line" expected formatted
  assertEq "structure field boundary has no trailing horizontal whitespace"
    formatted (Formatter.SpaceRules.stripTrailingWhitespace formatted)
...
def assertAmbiguousSyntaxChoiceRetainsSourceLayout (env : Lean.Environment)
    : IO Unit := do
  let source :=
    "def replayInstance before after basePrefix stepPrefix sameOperations event origin :=\n"
    ++ "  dispatch\n"
    ++ "    { before\n"
    ++ "      after\n"
    ++ "      basePrefix\n"
    ...
    ++ "      origin }\n"
  let sourceModule ←
    SyntaxTree.parseModuleStringWithEnv env source "ambiguous-structure-choice.lean"
  assertTrue "newline-separated shorthand fields retain Lean's syntax choice"
    (findTreeNode? (.raw `choice) sourceModule.tree).isSome
  let result ←
    Formatter.formatSourceWithEnvDetailed env source "ambiguous-structure-choice.lean"
  assertTrue "an ambiguous syntax choice does not fall back" (!result.fellBack)
  assertEq "an ambiguous syntax choice retains its source layout" source result.formatted
  assertTrue "an ambiguous syntax choice preserves code"
    (← codePreservedIgnoringWhitespace env source result.formatted)
  let formattedAgain ←
    Formatter.formatSourceWithEnv env result.formatted
      "ambiguous-structure-choice-formatted.lean"
  assertEq "ambiguous syntax choice formatting is idempotent"
    result.formatted formattedAgain
  ...
  IO.FS.withTempDir
    fun root => do
      ...
      let workerEnvironment :=
        LeanFmt.Driver.setWorkerEnvironmentVariable
          #[
            ("LEAN_PATH", some "/project/.lake/build/lib/lean"),
            ("LEANFMT_LOAD_DYNLIBS", some "/old/library.dylib")
          ]
          "LEANFMT_LOAD_DYNLIBS" (some "/target/library.dylib")
      assertTrue "worker environment carries one updated native-library request"
        (workerEnvironment
          == #[
            ("LEAN_PATH", some "/project/.lake/build/lib/lean"),
            ("LEANFMT_LOAD_DYNLIBS", some "/target/library.dylib")
          ])
      assertEq "runtime paths use the platform list separator"
        (toString [FilePath.mk "/first/library", FilePath.mk "/second/library"])
        (toString
          (LeanFmt.Driver.runtimePaths
...
def assertFormattingExceptionChecks (env : Lean.Environment) : IO Unit := do
  ...
  let structurallyIndentedIdentifier := String.ofList (List.replicate 37 'x')
  let fittingInfixSource :=
    s!"def shiftedInfix := left ++ {structurallyIndentedIdentifier}\n"
  let structurallyIndentedInfix :=
    "def shiftedInfix :=\n"
    ++ "  left\n"
    ++ String.ofList (List.replicate 64 ' ')
    ++ s!"++ {structurallyIndentedIdentifier}\n"
  let fittingInfixModule ←
    SyntaxTree.parseModuleStringWithEnv env fittingInfixSource
      "fitting-structural-infix-overflow.lean"
  let structurallyIndentedInfixModule ←
    SyntaxTree.parseModuleStringWithEnv env structurallyIndentedInfix
      "structural-infix-overflow.lean"
  assertTrue "an unbreakable token may overflow from its structural line indentation"
    (!(Formatter.Diagnostics.formattingExceptions
        fittingInfixModule structurallyIndentedInfixModule).any
        fun exception =>
          match exception with
          | .lineOverflow _ => true
          | _ => false)
...
def assertIndexedInfixRendersWithLeadingOperator (env : Lean.Environment) : IO Unit := do
  ...
  let nestedSource :=
    "syntax:50 term:51 \" =test[\" term \" ] \" term:51 : term\n"
    ++ "\n"
    ++ "def nestedIndexedRelation :=\n"
    ++ "  outerFunctionWithEnoughCharacters\n"
    ++ "    (leftOperandWithEnoughCharacters =test[index] rightOperandWithEnoughCharacters)\n"
  let nestedExpected :=
    "syntax:50 term:51 \" =test[\" term \" ] \" term:51 : term\n"
    ++ "\n"
    ++ "def nestedIndexedRelation :=\n"
    ++ "  outerFunctionWithEnoughCharacters\n"
    ++ "    (leftOperandWithEnoughCharacters\n"
    ++ "      =test[index] rightOperandWithEnoughCharacters)\n"
  let nestedFormatted ←
    Formatter.formatSourceWithEnv env nestedSource "nested-indexed-infix-base.lean"
      { lineWidth := 70 }
  assertEq "a parenthesized indexed infix uses a structural nested base"
    nestedExpected nestedFormatted
...
def assertLakeDslFormatting : IO Unit := do
  ...
  let source :=
    ...
    ++ "  extraDepTargets := #[`support]\n"
    ++ "  moreLinkArgs := if System.Platform.isOSX then #[] else #[\"-ldl\"]\n"
    ++ "\n"
    ++ "lean_exe quantum where\n"
    ++ "  root := `QuantumComputing\n"
    ++ "\n"
    ++ "extern_lib quantumffi (pkg) := do\n"
    ++ "  pure (pkg.staticLibDir / nameToStaticLib \"quantumffi\")\n"
  let expected :=
    "import Lake\n"
    ++ "\n"
    ++ "open Lake DSL\n"
...
def assertBorrowedTermUsesUnaryPrefixRule (env : Lean.Environment) : IO Unit := do
  let source := "def borrowedValue (value : @& Nat) : Nat := value\n"
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env source "borrowed-term-prefix.lean"
  assertTrue "borrowed terms have complete rule coverage"
    (Formatter.Diagnostics.missingRuleOccurrencesForModule moduleTree).isEmpty
  let formatted ← Formatter.formatSourceWithEnv env source "borrowed-term-prefix.lean"
  assertEq "borrowed terms use ordinary prefix formatting" source formatted

def assertParserDescribedSpacedTermsBecomeApplications (env : Lean.Environment)
    : IO Unit := do
  let source :=
    ...
    ++ "\n"
    ++ "def customPrefix := ~~~ veryLongPrefixOperandIdentifier\n"
    ++ "\n"
    ++ "def customOpaque := custom_opaque firstArgument => secondArgument\n"
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env source "parser-described-spaced-terms.lean"
  assertTrue "parser-described spaced terms own application trees"
  ...
  assertTrue "opaque parser-described syntax retains its missing-rule diagnostic"
    ((missing.any
        fun occurrence =>
          occurrence.syntaxKind? == some `customPunctuationPrefix)
      && (missing.any
            fun occurrence =>
              occurrence.syntaxKind? == some `customOpaqueTerm))
  let formatted ←
    Formatter.formatSourceWithEnv env source "parser-described-spaced-terms.lean"
      { lineWidth := 60 }
  assertTextContains "parser-described term arguments use application flow"
    formatted
    ("def customSpaced :=\n"
      ++ "  custom_spaced\n"
  ...
  assertTextContains "optional parser pieces do not hide application ownership"
    formatted
    ("def customOptional :=\n" ++ "  custom_optional veryLongOptionalOperandIdentifier\n")
  assertTrue "parser-described application grouping preserves code"
    (← codePreservedIgnoringWhitespace env source formatted)
  let formattedAgain ←
    Formatter.formatSourceWithEnv env formatted
      "parser-described-spaced-terms-formatted.lean" { lineWidth := 60 }
  assertEq "parser-described application grouping is idempotent" formatted formattedAgain
...
def assertParserDescribedSpacedIdentifierBecomesApplication (_env : Lean.Environment)
  ...
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env source
      "parser-described-spaced-identifier.lean"
  assertTrue "parser-described identifier suffix owns an application tree"
    ((findTreeNodeStartingWith? .application "in" moduleTree.tree).isSome)
  assertTrue "parser-described identifier suffix has complete rule coverage"
    (Formatter.Diagnostics.missingRuleOccurrencesForModule moduleTree).isEmpty
  let formatted ←
    Formatter.formatSourceWithEnv env source "parser-described-spaced-identifier.lean"
  assertEq "parser-described identifier suffix keeps its compact shape" source formatted
...
def assertParserDescribedDelimitedSyntaxUsesStructuralRules (_env : Lean.Environment)
  ...
  let source :=
    ...
    ++ "def generatedDelimited := project_generated(first, second)\n"
  let moduleTree ←
    SyntaxTree.parseModuleStringWithEnv env source
      "parser-described-delimited-syntax.lean"
  let config ←
    match findTreeNode? (.raw `projectDelimitedConfig) moduleTree.tree with
    | some tree => pure tree
    | none => throw <| IO.userError "project delimited config tree not found"
  let matrix ←
    match findTreeNode? (.raw `projectMatrixLiteral) moduleTree.tree with
    | some tree => pure tree
    | none => throw <| IO.userError "project matrix literal tree not found"
  let generated ←
    match findTreeNode? (.delimitedCollection .paren) moduleTree.tree with
    | some tree => pure tree
    | none => throw <| IO.userError "parser-annotated generated delimiter tree not found"
  assertTrue "parenthesized extension syntax uses the structural paren rule"
    ((Formatter.LineBreakRules.ruleFor config).any fun rule => rule.name == "paren")
  assertTrue "bracketed extension syntax uses the structural matrix rule"
    ((Formatter.LineBreakRules.ruleFor matrix).any
      fun rule => rule.name == "matrixNotation")
  assertTrue "parser-annotated generated delimiters use collection layout"
    ((Formatter.LineBreakRules.ruleFor generated).any fun rule => rule.name == "tuple")
  assertTrue "structurally delimited extension syntax has complete rule coverage"
    (Formatter.Diagnostics.missingRuleOccurrencesForModule moduleTree).isEmpty
  let formatted ←
    Formatter.formatSourceWithEnv env source "parser-described-delimited-syntax.lean"
  assertEq "fitting structurally delimited extension syntax stays compact"
    source formatted
  let longSource :=
    "def generatedAscription :=\n"
    ++ "  (veryLongValueWithEnoughCharactersForLayout :\n"
    ++ "    project_generated(veryLongFirstArgumentWithEnoughCharacters,\n"
    ++ "      veryLongSecondArgumentWithEnoughCharacters))\n"
  let longExpected :=
    "def generatedAscription :=\n"
    ++ "  (veryLongValueWithEnoughCharactersForLayout\n"
    ++ "    : project_generated(\n"
    ++ "        veryLongFirstArgumentWithEnoughCharacters,\n"
    ++ "        veryLongSecondArgumentWithEnoughCharacters\n"
    ++ "      ))\n"
  let longFormatted ←
    Formatter.formatSourceWithEnv env longSource
      "parser-described-long-delimited-syntax.lean" { lineWidth := 70 }
  assertEq "parser-described generated delimiters break structurally"
    longExpected longFormatted
  let longFormattedAgain ←
    Formatter.formatSourceWithEnv env longFormatted
      "parser-described-long-delimited-syntax-formatted.lean" { lineWidth := 70 }
  assertEq "parser-described generated delimiter formatting is idempotent"
    longFormatted longFormattedAgain
  let matrixSource :=
    "def longProjectMatrix := #pm[veryLongFirstMatrixEntry, veryLongSecondMatrixEntry; veryLongThirdMatrixEntry, veryLongFourthMatrixEntry]\n"
  let matrixExpected :=
    "def longProjectMatrix :=\n"
    ++ "  #pm[\n"
    ++ "    veryLongFirstMatrixEntry,\n"
    ++ "    veryLongSecondMatrixEntry;\n"
    ++ "    veryLongThirdMatrixEntry,\n"
    ++ "    veryLongFourthMatrixEntry\n"
    ++ "  ]\n"
  let matrixFormatted ←
    Formatter.formatSourceWithEnv env matrixSource
      "parser-described-long-matrix-syntax.lean" { lineWidth := 60 }
  assertEq "a wide matrix uses balanced item and delimiter breaks"
    matrixExpected matrixFormatted
...
def assertTightIndexedExtensionUsesStructuralRule (_env : Lean.Environment)
  ...
  let indexed ←
    match findTreeNode? (.raw `projectTightIndexed) moduleTree.tree with
    | some tree => pure tree
    | none => throw <| IO.userError "tight indexed extension tree not found"
  assertEq "tight indexed extension syntax uses the indexed-term rule"
    "indexedTerm" (Formatter.LineBreakRules.formattingRuleFor indexed).name
  assertTrue "tight indexed extension syntax has complete rule coverage"
    (Formatter.Diagnostics.missingRuleOccurrencesForModule moduleTree).isEmpty
...
def assertFallbackAndConditionalSuffixesStayAttached (env : Lean.Environment)
    : IO Unit := do
  let fallbackSource :=
    "def fallbackSuffix := do\n"
    ++ "  let some (first, second, third, fourth) := shape? |\n"
    ++ "    -- The fallback comment starts the fallback body.\n"
    ++ "    failure\n"
    ++ "  pure first\n"
  let fallbackExpected :=
    "def fallbackSuffix := do\n"
    ++ "  let some (first, second, third, fourth) := shape? |\n"
    ++ "    -- The fallback comment starts the fallback body.\n"
    ++ "    failure\n"
    ++ "  pure first\n"
  let fallbackResult ←
    Formatter.formatSourceWithEnvDetailed env fallbackSource
      "attached-fallback-suffix.lean" { lineWidth := 100 }
  assertTrue "attached fallback suffix does not fall back" (!fallbackResult.fellBack)
  assertEq "a fitting fallback bar stays with its declaration"
    fallbackExpected fallbackResult.formatted
  assertTrue "attached fallback suffix preserves code"
    (← codePreservedIgnoringWhitespace env fallbackSource fallbackResult.formatted)
  let fallbackAgain ←
  ...
  let conditionalSource :=
    ...
    ++ "  if firstCondition then firstValue else if secondCondition then secondValue else do\n"
    ++ "    pure thirdValue\n"
  let conditionalExpected :=
    "def conditionalSuffix :=\n"
    ++ "  if firstCondition then\n"
    ...
    ++ "  else do\n"
    ++ "    pure thirdValue\n"
  let conditionalFormatted ←
    Formatter.formatSourceWithEnv env conditionalSource
      "attached-else-do-suffix.lean" { lineWidth := 50 }
  assertEq "an else-if chain keeps its final do suffix attached"
    conditionalExpected conditionalFormatted
  assertTrue "an attached else-do suffix preserves code"
    (← codePreservedIgnoringWhitespace env conditionalSource conditionalFormatted)
  let conditionalAgain ←
    Formatter.formatSourceWithEnv env conditionalFormatted
      "attached-else-do-suffix-formatted.lean" { lineWidth := 50 }
...
def assertReviewedMathlibContinuationAndSuffixes (_env : Lean.Environment) : IO Unit := do
  ...
  check "reviewed-else-if-header-suffix-after-earlier-branches"
    ("def reviewedMapField (n : Name) (cl f alpha beta e : Expr) : TermElabM Expr := do\n"
      ++ "  let t <- whnf (<- inferType e)\n"
      ++ "  if t.getAppFn.constName = some n then\n"
      ++ "    throwError \"recursive types not supported\"\n"
      ++ "  else if alpha.eqv e then\n"
      ++ "    return beta\n"
      ++ "  else if alpha.occurs t then\n"
      ++ "    let f' <- nestedMap f alpha t\n"
      ++ "    return f'.app e\n"
      ++ "  else if\n"
      ++ "    <- (match t with\n"
      ++ "        | .app t' _ => withNewMCtxDepth <| isDefEq t' cl\n"
  ...
  check "reviewed-nested-conditional-infix-spacing"
    ("def reviewedNestedConditionalSpacing := do\n"
      ++ "  if (firstConditionWithEnoughCharactersToRequireBreaking\n"
      ++ "        && if let some value := optionalValueWithEnoughCharacters then\n"
      ++ "              predicateWithEnoughCharacters value\n"
      ++ "            else\n"
      ++ "              true)\n"
      ++ "      || secondConditionWithEnoughCharactersToRequireBreaking then\n"
      ++ "    return firstResult\n"
      ++ "  else\n"
      ++ "    return secondResult\n")
    ("def reviewedNestedConditionalSpacing := do\n"
...
def assertOptionalAccessSuffixPropagatesApplicationFit (env : Lean.Environment)
    : IO Unit := do
  let source :=
    "theorem optionalAccessApplicationFit\n"
  ...
  let expected :=
    ...
    ++ "    : (berlekampMatrix f hmonic)[i][j]\n"
    ++ "      = ((berlekampColumnPolys f hmonic (FpPoly.frobeniusXMod f hmonic)\n"
    ++ "            (basisSize f) 1 #[])[j.val]?.getD\n"
    ++ "          0).coeff\n"
    ++ "          i.val := by\n"
    ++ "  rw [berlekampMatrix, Matrix.getElem_ofFn]\n"
  let result ←
    Formatter.formatSourceWithEnvDetailed env source
      "optional-access-application-fit.lean" { lineWidth := 100 }
  assertTrue "optional-access suffix fitting does not fall back" (!result.fellBack)
  assertEq "tight optional access participates in application fit" expected
    result.formatted
  assertTrue "optional-access application formatting fits its width"
    (Formatter.linesFit result.formatted 100)
  assertTrue "optional-access application formatting preserves code"
    (← codePreservedIgnoringWhitespace env source result.formatted)
  let formattedAgain ←
    Formatter.formatSourceWithEnv env result.formatted
      "optional-access-application-fit-formatted.lean" { lineWidth := 100 }
...
def runSyntaxTreeTests (env : Lean.Environment) : IO Unit := do
  ...
  assertParserDescribedSpacedIdentifierBecomesApplication env
  assertParserDescribedDelimitedSyntaxUsesStructuralRules env
...
def runBasicFormattingTests (env : Lean.Environment) : IO Unit := do
  ...
  assertOpenCommandListBreaksFromCommandBase env
  assertPrintAxiomsUsesCommandHead env
  ...
  assertAssignedProofKeepsByWithAssignment env
  assertTermTakingTacticsAttachOperandHead env
...
def runExpressionAndRendererTests (env : Lean.Environment) : IO Unit := do
  ...
  assertVariableBinderMovesAsPeerBeforeInternalBreak env
  ...
  assertIntervalBinderBreaksBetweenRangeEndpoints env
  assertIndexedInfixBinderBodyUsesStructuralBase env
  assertInfixIgnoresFittingSourceBreaks env
  assertOrdinarySyntaxNormalizesSourceTightSpacing env
  assertMultiplicationChainsFlow env
  assertInfixIgnoresArbitrarySourceBreaks env
  assertBlankSourceTriviaDoesNotDetachSyntax env
...
def runCollectionAndDeclarationTests (env : Lean.Environment) : IO Unit := do
  ...
  assertStructureFieldBoundaryPreservesOneCleanBlankLine env
  ...
  assertAmbiguousSyntaxChoiceRetainsSourceLayout env
