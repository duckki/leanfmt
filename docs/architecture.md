...
| `.application` | Lean parser applications are nested per argument, but formatting wants one function-application segment. Generated term parsers can express the same shape as a head and arguments separated by empty `ppSpace` groups; recognizing that structural pattern prevents continuation indentation from staircasing through the generated wrapper. Applications always own their physical start column, including applications on ordinary infix, pipe-projection, and low-priority-infix operands. `<|` is exceptional only in its breakpoint policy, not in application-base selection. | Child `0` is the head, children `1...` are arguments in source order. Raw `null` argument containers are spliced. A generated spaced form requires at least two non-atomic argument trees and exactly one empty generated group between every content child; ambiguous generated token sequences remain extension-owned. `return` wrappers move their prefix into a `.suffixGroup` with the application head, so ordinary and lambda arguments share the prefix's structural base instead of the function name's inline column. |
...
emission. A regrouped proof body stops inherited suffix measurement after its `by`
...
Proof subtrees and attribute instances are not reformatted.
