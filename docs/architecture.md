...
| `.ifThenElseClause` and `.ifThenElseChain kind` | Ordinary, dependent, pattern, and `do`-notation continuations retain conditional ownership independently of source comments. Layout preparation joins uninterrupted runs for the existing balanced branch rule. The parser kind still distinguishes mandatory `doIf` layout. | A syntax chain contains a clause header, its result branch, `else`, and a complete nested continuation. The last conditional instead owns its fallback. Headers contain `if` plus a suffix group holding the condition and `then`; named and pattern conditions retain their existing header structure. Attached `do`/`by` bodies join the header suffix and leave the branch peer empty, and final `else` suffix attachment is unchanged. Parser-packed `do` continuations are nested into the same ownership shape. `LayoutTree` flattens only links without source-forced comment breaks into alternating header/body peers; it prepends `else` to the following header in that view only. Single conditionals outside chains retain their existing raw rules. |
