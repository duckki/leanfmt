...
2. Parse the header and commands with Lean's module parser. The default public API
   ...
   and let parser-owned trailing keyword clauses expose their bodies without putting
   parser metadata in line-break rules. The CLI first tries
...
| `.parserOwnedHeader` and `.parserOwnedBody` | A registered command or tactic parser can declare a trailing keyword clause whose final term is a body. Exposing that ownership keeps the keyword on the header and gives the body one structural continuation base without making line-break rules inspect parser kinds, token spacing, or project-specific names. Only keyword-like suffixes from active `command` and `tactic` parser descriptions qualify; the concrete suffix must occur in the parsed node, so absent optional clauses and punctuation-only syntax remain opaque. Term parsers keep their missing-rule diagnostics. | The body node contains one header followed by the complete term body. A nonempty parser-described prefix becomes `.parserOwnedHeader`; its final content child and the suffix form a `.suffixGroup`, while preceding header pieces retain ordinary flow boundaries. This lets a long tactic header wrap before its arguments without detaching a suffix such as `using`. Attached `do` or `by` bodies exposed by existing structural syntax use the body node with their existing header shape. In a protected proof, an inline header and body remain compact; a source-detached body marks the tactic as a layout owner so its continuation can be normalized. |
...
keyword/body suffixes reachable from registered command or tactic parsers. Descriptor
