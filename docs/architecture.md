...
- `keepPrefixWithChildFirstLine`: at a selected child boundary, when the child can format
  ...
  breakpoint. If a tight leaf suffix does not fit and the flow has earlier structural
  breaks, the renderer applies those earlier breaks instead of detaching the suffix.
